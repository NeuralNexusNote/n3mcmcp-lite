...
class TestToolRegistration:
    def test_seven_tools_registered(self):
        ...
        # §4.3 + §4.3.1: 6 base tools + recall_thread (Retrieval Extensions).
        assert names == {
            "search_memory",
            "save_memory",
            "list_memories",
...
class TestSaveAndSearch:
    ...
    def test_code_blocks_purified_in_search_results(self, db):
        """§10.3 purification contract: code blocks in recalled content → [code omitted]."""
        import asyncio
        from n3mc_mcp.server import call_tool
        self._setup_db(db)
        content = "機械学習メモ\n```python\nprint('hello world')\n```\n以上がサンプルコードです。"
        asyncio.run(call_tool("save_memory", {"content": content}))
        import time; time.sleep(0.5)
        result = asyncio.run(call_tool("search_memory", {"query": "機械学習 サンプルコード"}))
        text = result[0].text
        assert "[code omitted]" in text, "purify() not applied: code block still present in response"
        assert "```" not in text.split("---")[0], "raw code fence leaked into search result"
