...
**揮発性の契約：**
- Redis への全書き込みで `ttl_seconds`（既定 86 400 = 24 時間）の TTL を設定する。
...
**TTL**：`mem:<uuid>` への `HSET` は毎回 `EXPIRE mem:<uuid> <ttl_seconds>` と（`PIPELINE` でアトミックに）組み合わせる。兄弟である `mem:sha:<sha1>` ガードは同じパイプライン内で `SET ... EX <ttl_seconds>` にて書き込む。既定 TTL は 86 400 秒（24 時間）。
...
4. **セッション間テスト（24 時間以内）** — セッション 1 で保存、MCP サーバーを再起動（Redis は再起動しない）、セッション 2 で検索。保存エントリが取得できることを確認。
