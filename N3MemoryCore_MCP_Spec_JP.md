...
> **N3MC シリーズの主たる利用方法は「指示書（本仕様書）から AI に実装を生成させる」ことです。** 本仕様書は AI による再生成を前提に詳細化されており（冒頭「以降は Pro 版と同じ構造で記述し、AI による再生成を容易にしています」）、Claude 等に読み込ませることで実装・カスタマイズ・再生成ができます。**pip / マーケットプレイスでのパッケージ導入は、これを手早く試すための補助的な「おまけ」機能**です。

どちらの方法でも、先に **Redis Stack を起動**しておきます：
...
1. **Redis Stack を起動**（上記）。
2. **本仕様書を AI に読み込ませて実装を依頼**：`N3MemoryCore_MCP_Spec_JP.md`（または `_EN.md`）を Claude 等に渡し、「この指示書に従って n3mcmcp-lite を実装してください」と依頼します。AI がパッケージ構成・RediSearch インデックス作成・MCP ツール登録・stdio 起動まで生成します。段階的な **実装 → デバッグ → 自律評価 → 品質レビュー** の 4 フェーズ・ワークフローは [付録 B](#付録-b推奨-ai-支援ワークフロー) を参照。
3. **生成物を導入**：プロジェクトルートで `pip install -e .` した後、MCP クライアント設定にサーバーを登録（[§8](#8-mcp-クライアント設定)）。
4. **クライアントを再起動**。初回ツール呼び出しは ~400 MB の埋め込みモデルのダウンロードとロードで 30–60 秒かかります。

#### 方法 B：パッケージ導入（おまけ）

公開済みパッケージをそのまま入れて手早く動かしたい場合はこちら：
...
doc:<uuid>                  HASH（親ドキュメント — 長文 verbatim 保存用。RediSearch 非インデックス）
    ...
    chunk_ids       string      子チャンク id の空白区切りリスト（兄弟チャンク TTL 同時延長用）
    TTL                         ttl_seconds
...
**`min_score` フィルタは語彙リランクの後に適用する。** 融合スコアが `min_score` 直下の候補でも、フレーズ一致・coverage によるリランク加点で閾値を超えれば結果に残せるよう、スコアフロアによる足切りは融合時点ではなくリランク**後**に行う。融合時点で枝刈りすると、埋め込みが弱いがクエリ句を完全に含むレコードをリランクが救済する前に捨ててしまうため、この順序が重要である（[§6](#6-設定) `min_score`）。
...
- `ttl_refresh_on_search` が有効なとき、TTL リフレッシュはリランク**後の**上位 K 件に適用される：各ヒットの `mem:<chunk_id>`（または独立 `mem:<id>`）キーの `access_count` が +1 され、親ドキュメントに解決されたヒットでは `doc:<parent_id>` の TTL と、**`chunk_ids` に列挙された全兄弟チャンク `mem:<chunk_id>` の TTL を一括で同時リフレッシュする**（最高スコアの 1 チャンクだけでなく、その文書を構成する全チャンクが揃って延命する）。これにより、ある兄弟チャンク固有の語句で後続検索した場合の検索性が窓全体で保たれる（最高スコアチャンクのみを延長すると、他チャンクが窓途中で失効して検索ヒット経路が痩せる問題を解消）。`access_count` のインクリメントは実際にヒットしたチャンクにのみ適用し、サイレントに延命される兄弟チャンクには加算しない。`chunk_ids` フィールドを持たない旧 doc（アップグレード前に保存）は最高スコアチャンク 1 個＋親のみの延長に graceful にフォールバックする。
...
- 親が生存している限り、チャンクが 1 個でもヒットすれば全文が返る。`ttl_refresh_on_search` が有効（既定 `true`）な場合、チャンクがヒットして `doc:` キーを取得するたびに、親の TTL と `chunk_ids` に列挙された全兄弟チャンクの TTL が同時にリフレッシュされるため、通常の利用では親も子チャンク群も揃って延命し、文書を構成するどのチャンクも窓途中で失効しない。万一 `ttl_refresh_on_search: false` 設定または初回 7 日経過後など親の TTL が切れた場合、孤児チャンクは個別メモリとして（短文ヒットの形で）表示される（退行ではなく graceful degrade）。
...
**サニタイズ対象に含まれないもの（意図的）** — 以下は **保存・取り出しを通じて
verbatim に保持される**（§3.11 の親ドキュメント verbatim 契約と整合）。表示・
グラフ可逆性・異体字情報・後方互換のため、変換を行わない:

- **`U+0000`（NUL）等の C0 制御文字** — Redis HSET / RediSearch HASH はバイナリ
  セーフのため、NUL を含む文字列もそのまま round-trip する。`A\x00B` のような
  入力は 3 文字として保存される（empty-content 拒否経路には進まない）。
- **ゼロ幅スペース・結合用スペーサ**（`U+200B`、`U+200C`、`U+200D`、`U+FEFF` 等） —
  検索時のトークナイズには影響しうるが（RediSearch 側の挙動）、保存本文には残る。
- **異体字セレクタ**（`U+E0100`–`U+E01EF`、`U+FE00`–`U+FE0F`） — `神󠄀` のような
  字形指定が **verbatim** で保たれる（CJK 拡張と組み合わせた異体字情報も同様）。
- **結合用ダイアクリティクス**（`U+0300`–`U+036F`、`U+3099`–`U+309A` 等） — 入力が
  decomposed 形式の場合、保存本文も decomposed のまま。NFKC 正規化は **embedding
  生成と dedup hash 計算のみ**に適用されるため（§3.7 / §3.8）、検索・重複判定は
  decomposed と precomposed を同一視するが、`list_memories` / `search_memory` 応答
  は入力形式を尊重する。

つまり「`save_memory` に渡したバイト列は、サロゲートだけ落として、それ以外は
丸ごと帰ってくる」という最小限のサニタイズ契約を採る。これは silent な変換が
verbatim 契約を破る可能性を最小化するための設計判断である。
...
- `min_score` — このスコア未満の結果を除外（既定 `0.2`）。`0.0` で無効化。**フィルタは語彙リランク（[§3.6](#36-ランキング式)）の後に適用される** — フレーズ一致・coverage の加点を受けた最終スコアで判定するため、融合スコアが閾値直下でもリランク加点で救済されうる。
